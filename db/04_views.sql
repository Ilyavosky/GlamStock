...
SELECT
  ...
  RANK() OVER (ORDER BY COALESCE(SUM(vb.cantidad), 0) DESC) AS ranking_mas_vendido,
  RANK() OVER (ORDER BY COALESCE(SUM(vb.cantidad), 0) ASC, pm.nombre ASC, v.modelo ASC, v.color ASC) AS ranking_menos_vendido
FROM variantes v
JOIN productos_maestros pm ON v.id_producto_maestro = pm.id_producto_maestro
...
SELECT
  ...
  COALESCE(ROUND(SUM((vb.precio_venta_final - v.precio_adquisicion) * vb.cantidad), 2), 0) AS utilidad_total,
  RANK() OVER (ORDER BY COALESCE(SUM(vb.cantidad), 0) DESC) AS ranking_mas_vendido,
  RANK() OVER (ORDER BY COALESCE(SUM(vb.cantidad), 0) ASC,
                pm.nombre ASC, v.modelo ASC, v.color ASC) AS ranking_menos_vendido
FROM variantes v
JOIN productos_maestros pm ON v.id_producto_maestro = pm.id_producto_maestro
LEFT JOIN ventas_bajas  vb ON vb.id_variante = v.id_variante
GROUP BY
...
LEFT JOIN ventas_bajas  vb ON vb.id_variante = v.id_variante AND vb.id_sucursal = s.id_sucursal
GROUP BY
...
CREATE UNIQUE INDEX idx_ranking_sucursal_variante
  ON vista_ranking_productos_por_sucursal(id_variante);
CREATE INDEX idx_ranking_sucursal_mas
  ON vista_ranking_productos_por_sucursal(ranking_mas_vendido);
CREATE INDEX idx_ranking_sucursal_menos
  ON vista_ranking_productos_por_sucursal(ranking_menos_vendido);
CREATE INDEX idx_ranking_sucursal_producto
  ON vista_ranking_productos_por_sucursal(id_producto_maestro);
